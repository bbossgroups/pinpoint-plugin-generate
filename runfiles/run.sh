...
case \$1 in
   start)
    nohup java \$RT_JAVA_OPTS -jar ${project}-bootstrap-${bboss_version}.jar > ${project}.log &
    tail -f ${project}.log
     ;;
   stop)
    java -jar ${project}-bootstrap-${bboss_version}.jar  stop --shutdownLevel=9
     ;;
   restart)
    nohup java \$RT_JAVA_OPTS -jar ${project}-bootstrap-${bboss_version}.jar restart --shutdownLevel=9 > ${project}.log &
    tail -f ${project}.log
    ;;
   *)
     echo
